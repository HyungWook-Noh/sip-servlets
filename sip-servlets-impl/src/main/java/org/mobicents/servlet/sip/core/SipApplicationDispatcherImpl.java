...
//			analyzeCPU();
	...
	public void init() throws IllegalArgumentException {
		...
		asynchronousScheduledThreadPoolExecutor = new ScheduledThreadPoolExecutor(2,
...
//									sipServletResponse.send();
	...
	public void processTransactionTerminated(final TransactionTerminatedEvent transactionTerminatedEvent) {
		...
		final Transaction transaction = eventTransaction;
		if(logger.isDebugEnabled()) {
			logger.debug("transaction " + transaction + " terminated => " + transaction.getRequest().toString());
		}		
