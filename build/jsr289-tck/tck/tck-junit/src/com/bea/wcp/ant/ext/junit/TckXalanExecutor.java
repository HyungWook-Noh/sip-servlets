 ...
 */
abstract class TckXalanExecutor {
    private static final String pack = 
        "com.bea.wcp.ant.ext.junit.";
    private static final String apache_pack = 
    	"org.apache.tools.ant.taskdefs.optional.junit.";

    /** the transformer caller */
    protected TckAggregateTransformer caller;

    /** set the caller for this object. */
    private final void setCaller(TckAggregateTransformer caller) {
        this.caller = caller;
    }
     ...
     * executor.
     */
    static TckXalanExecutor newInstance(TckAggregateTransformer caller) 
        throws BuildException {
        TckXalanExecutor executor = null;
        try {
            Class clazz = Class.forName(pack + "TckXalan2Executor");
            executor = (TckXalanExecutor)clazz.newInstance();
        } catch (Exception xsltcApacheMissing){
            caller.task.log(xsltcApacheMissing.toString());
            try {
                Class clazz = Class.forName(apache_pack + "Xalan1Executor");
                executor = (TckXalanExecutor) clazz.newInstance();
