 ...
 * @see BatchTest 
 */
public class TckJUnitTask extends Task {
    ...
    private TckJUnitTestRunner runner = null;
     ...
     * @since Ant 1.2
     */
    public void addTest(TckJUnitTest test) {
     ...
     * @since Ant 1.2
     */
    public void addFormatter(TckFormatterElement fe) {
     ...
     * @since Ant 1.2
     */
    public TckJUnitTask() throws Exception {
        getCommandline()
...
//        	System.out.println("$$$$$$$$$in TckJUnit execute() 2");
    ...
    /**
     * Run the tests.
     * @param arg one TckJunitTest
     * @throws BuildException in case of test failures or errors
     */
    protected void execute(TckJUnitTest arg) throws BuildException {
        TckJUnitTest test = (TckJUnitTest) arg.clone();
        ...
        int exitValue = TckJUnitTestRunner.ERRORS;
     ...
     */
    private int executeAsForked(TckJUnitTest test, ExecuteWatchdog watchdog, 
                                File casesFile)
        ...
        cmd.setClassname("com.bea.wcp.ant.ext.junit.TckJUnitTestRunner");
//        cmd.setClassname("org.apache.tools.ant.taskdefs.optional.junit.TckJUnitTestRunner");
        if (casesFile == null) {
        ...
        final TckFormatterElement[] feArray = mergeFormatters(test);
        for (int i = 0; i < feArray.length; i++) {
            TckFormatterElement fe = feArray[i];
     ...
     */
    private int executeInVM(TckJUnitTest arg) throws BuildException {
        TckJUnitTest test = (TckJUnitTest) arg.clone();
        ...
        try {
            ...
            runner = new TckJUnitTestRunner(test, test.getHaltonerror(),
                                         test.getFiltertrace(),
     ...
     * @since Ant 1.3
     */
    private TckFormatterElement[] mergeFormatters(TckJUnitTest test) {
        Vector feVector = (Vector) formatters.clone();
        test.addFormattersTo(feVector);
        TckFormatterElement[] feArray = new TckFormatterElement[feVector.size()];
     ...
     * @since Ant 1.3
     */
    protected File getOutput(TckFormatterElement fe, TckJUnitTest test) {
        if (fe.getUseFile()) {
            String base = test.getOutfile();
            if (base == null) {
                base = TckJUnitTestRunner.IGNORED_FILE_NAME;
            }
    ...
    private void logTimeout(TckFormatterElement[] feArray, TckJUnitTest test) {
        createClassLoader();
     ...
     * @since Ant 1.6
     */
    private void addTimeout(TckJUnitTest test, JUnitResultFormatter formatter,
                            OutputStream out) {
    ...
    private final class ForkedTestConfiguration {
         ...
         * @param test
         */
        ForkedTestConfiguration(TckJUnitTest test) {
    ...
    protected Collection executeOrQueue(Enumeration testList,
        ...
        while (testList.hasMoreElements()) {
        	Object tmp = testList.nextElement();
//        	System.out.println("-------------"
//        			+ "in TckJUnitTask.executeOrQueue(Enumeration, boolean)"
//        			+ tmp.getClass().getName());
            TckJUnitTest test = (TckJUnitTest) tmp;
    ...
    protected void actOnTestResult(int exitValue, boolean wasKilled,
                                   TckJUnitTest test, String name) {
        // if there is an error/failure and that it should halt, stop
        // everything otherwise just log a statement
        boolean errorOccurredHere =
            exitValue == TckJUnitTestRunner.ERRORS || wasKilled;
        boolean failureOccurredHere =
            exitValue != TckJUnitTestRunner.SUCCESS || wasKilled;
