...
{
    /**
     * Comment for <code>SIPUNIT_INTERNAL_RETURNCODE_MIN</code> A constant
    ...
    static
    {
        ...
        statusCodeDescription.put(new Integer(EXCEPTION_ENCOUNTERED),
                "Exception Encountered");
        statusCodeDescription.put(new Integer(UNSUPPORTED_URI_SCHEME),
                "Unsupported URI Scheme");
        statusCodeDescription.put(new Integer(INVALID_UNREGISTERED_OPERATION),
                "Invalid Unregistered Operation");
        statusCodeDescription.put(new Integer(TIMEOUT_OCCURRED),
                "Timeout Occurred");
        statusCodeDescription.put(new Integer(FAR_END_ERROR), "Far End Error");
        statusCodeDescription
                .put(new Integer(INTERNAL_ERROR), "Internal Error");
        statusCodeDescription.put(new Integer(ERROR_OF_UNKNOWN_ORIGIN),
                "Error of Unknown Origin");
        statusCodeDescription.put(new Integer(INVALID_OPERATION),
                "Invalid Operation");
        statusCodeDescription.put(new Integer(INVALID_ARGUMENT),
                "Invalid Argument");
    }
            ...
            ParseException
    {
        ...
        this.myhost = parent.getSipProvider().getListeningPoints()[0]
                .getIPAddress();
        ...
        if (tokens.countTokens() != 2)
        {
            throw new InvalidArgumentException(
                    "The \'me\' parameter must be in name@host format");
        }
        ...
        {
            throw new InvalidArgumentException("URI " + me
                    + " is not a Sip URI");
        }
        ...
        // (use user@hostname)
        SipURI contact_uri = addr_factory.createSipURI(((SipURI) my_uri)
                .getUser(), this.myhost);

        contact_uri.setPort(parent.getSipProvider().getListeningPoints()[0]
                .getPort());
        contact_uri.setTransportParam(parent.getSipProvider()
        ...
        ContactHeader hdr = parent.getHeaderFactory().createContactHeader(
                contact_address);
        ...
        ViaHeader via_header = parent.getHeaderFactory().createViaHeader(
                this.myhost,
    ...
    /**
     * This method idles this SipSession. This SipSession object must not be
     * used again after calling the dispose() method.
     * 
     */
    public void dispose()
    {
    ...
    /**
     * The getParent() method returns the org.cafesip.sipunit.SipStack object
     * that is associated with this SipSession object. A test program can use
     * the SipStack object to get the header, address, and message factories for
     * building requests and responses, before passing those to methods of this
     * class.
     * 
    ...
    /**
     * This method returns the IP address and port currently being used in this
     * Sip agent's contact address, via, and listening point 'sentby'
     * components. Example: 66.32.44.114:5066
     * 
    ...
    /**
     * This method replaces the host/port values currently being used in this
     * Sip agent's contact address, via, and listening point 'sentby' components
     * with the given host and port parameters.
     * 
     * Call this method when you are running a SipUnit testcase behind a NAT and
     * need to register with a proxy on the public internet. Before creating the
     * SipStack and SipPhone, you'll need to first obtain the public IP
     * address/port.
     * 
     * @param host
     *            The publicly accessible IP address for this Sip client (ex:
     *            66.32.44.112).
     * @param port
     *            The port to be used with the publicly accessible IP address.
     * @return true if the parameters are successfully parsed and this client's
     *         information is updated, false otherwise.
     */
    public boolean setPublicAddress(String host, int port)
    {
        try
        {
            ...
            parent.getSipProvider().getListeningPoints()[0].setSentBy(host
                    + ":" + port);

            // update my contact info
            SipURI my_uri = (SipURI) contactInfo.getContactHeader()
                    .getAddress().getURI();
            my_uri.setHost(host);
            my_uri.setPort(port);
    ...
    public void processRequest(RequestEvent request)
    {
        ...
        {
            my_contact_info.setContactHeader((ContactHeader) (contactInfo
        ...
        SipStack.trace("     my local contact info ('Request URI' check) = "
                + my_contact_info.getURI());
        ...
        synchronized (reqBlock)
        {
            ...
            {
                SipStack
                        .trace("SipSession: not interested in blocking requests");
                return;
            }
    ...
    protected static boolean destMatch(SipURI uri1, SipURI uri2)
    {
        if (uri1.getScheme().equalsIgnoreCase(uri2.getScheme()))
        {
            ...
            /*
             * if (uri1.getTransportParam() != null) { if
             * (uri2.getTransportParam() == null) { return false; }
             * 
             * if (uri1.getTransportParam().equals(uri2.getTransportParam()) ==
             * false) { return false; } } else if (uri2.getTransportParam() !=
             * null) { return false; }
             * 
             * if (uri1.getTTLParam() != -1) { if (uri2.getTTLParam() == -1) {
             * return false; }
             * 
             * if (uri1.getTTLParam() != uri2.getTTLParam()) { return false; } }
             * else if (uri2.getTTLParam() != -1) { return false; }
             * 
             ...
             * uri-parameter appearing in both URIs must match. - A user, ttl,
             * or method uri-parameter appearing in only one URI never matches,
             * even if it contains the default value. - A URI that includes an
             * maddr parameter will not match a URI that contains no maddr
             * parameter. - All other uri-parameters appearing in only one URI
             * are ignored when comparing the URIs.
             * 
    ...
    /**
     * This sendUnidirectionalRequest() method sends out a request message with
     * no response expected. A Request object is constructed from the string
     * parameter passed in.
     * 
     * Example: <code>
     ...
     * 
     * @param reqMessage
     *            A request message in the form of a String with everything from
     *            the request line and headers to the body. It must be in the
     *            proper format per RFC-3261.
     * 
     * @param viaProxy
     *            If true, send the message to the proxy. In this case the
     ...
     *             if an error was encountered while parsing the string.
     */
    ...
    /**
     * This sendUnidirectionalRequest() method sends out a request message with
     * no response expected. The Request object passed in must be a fully formed
     * Request with all required content, ready to be sent.
     * 
     * @param request
    ...
    private boolean addProxy(Request request)
    {
        if (proxyHost == null)
        {
            errorMessage = "Attempt to add Route header for proxy, but the proxy server was not specified for this SipPhone";
        ...
        try
        {
            SipURI route_uri = parent.getAddressFactory().createSipURI(null,
                    proxyHost);
            ...
            Address route_address = parent.getAddressFactory().createAddress(
                    route_uri);
            request.addHeader(parent.getHeaderFactory().createRouteHeader(
                    route_address));
    ...
    /**
     * This basic method sends out a request message as part of a transaction. A
     * test program should use this method when a response to a request is
     * expected. A Request object is constructed from the string passed in.
     * 
     * This method returns when the request message has been sent out. The
     * calling program must subsequently call the waitResponse() method to wait
     * for the result (response, timeout, etc.).
     * 
     * @param reqMessage
     *            A request message in the form of a String with everything from
     *            the request line and headers to the body. It must be in the
     ...
     *            routing to complete. This method does NOT add a route header.
     * @param dialog
     *            If not null, send the request via the given dialog. Else send
     *            it outside of any dialog.
     * @return A SipTransaction object if the message was built and sent
     *         successfully, null otherwise. The calling program doesn't need to
     *         do anything with the returned SipTransaction other than pass it
     *         in to a subsequent call to waitResponse().
     * @throws ParseException
     *             if an error is encountered while parsing the string.
     */
    public SipTransaction sendRequestWithTransaction(String reqMessage,
            boolean viaProxy, Dialog dialog) throws ParseException
    {
        Request request = parent.getMessageFactory().createRequest(reqMessage);
     ...
     * This method is the same as the basic
     * sendRequestWithTransaction(String,...) method except that it allows the
     * caller to specify a message body and/or additional message headers to add
     * to or replace in the outbound message without requiring knowledge of the
     * JAIN-SIP API.
     * 
     * The extra parameters supported by this method are:
     * 
            ...
            ArrayList replaceHeaders) throws ParseException
    {
        ...
        {
            return sendRequestWithTransaction(request, viaProxy, dialog,
                    toHeader(additionalHeaders, contentType, contentSubType),
     ...
     * This basic method sends out a request message as part of a transaction. A
     * test program should use this method when a response to a request is
     * expected. The Request object passed in must be a fully formed Request
     * with all required content, EXCEPT for the Via header branch parameter,
     * which cannot be filled in until a client transaction is obtained. That
     * happens in this method. If the Via branch value is set in the request
     * parameter passed to this method, it is nulled out by this method so that
     * a new client transaction can be created by the stack.
     * 
     * This method returns when the request message has been sent out. The
     * calling program must subsequently call the waitResponse() method to wait
     * for the result (response, timeout, etc.).
    ...
    /**
     * This method is the same as the basic
     * sendRequestWithTransaction(Request,...) method except that it allows the
     * caller to specify a message body and/or additional JAIN-SIP API message
     * headers to add to or replace in the outbound message. Use of this method
     * requires knowledge of the JAIN-SIP API.
     * 
    ...
    /**
     * This method is like other sendRequestWithTransaction() except that it
            ...
            ArrayList additionalHeaders, ArrayList replaceHeaders, String body)
    {
        ...
        try
        {
            ...
            ClientTransaction trans = parent.getSipProvider()
            ...
            try
            {
                if (dialog == null || Request.CANCEL.equals(request.getMethod()))
                {
                    trans.sendRequest();
                }
                else
     ...
     * transactional request message. Call this method after using one of the
     * sendRequestWithTransaction() methods.
     * 
    ...
    /**
     * This method prepares the SipSession for reception of a request message
     * addressed to this SipSession's URI. It is non-blocking and returns
     * immediately. A test program must subsequently call the waitRequest()
     * method which blocks until a request is received for this session's URI.
     * 
    ...
    /**
     * The unlistenRequestMessage() method cancels out a previous directive to
     * listen for reception of a request addressed to this SipSession's URI.
     * That is, it undoes a previous call to listenRequestMessage().
     * 
    ...
    /**
     * The waitRequest() method waits for a request addressed to this
     * SipSession's URI to be received from the network. Call this method after
     * calling the listenRequestMessage() method.
     * 
     * This method blocks until one of the following occurs: 1) A
     * javax.sip.RequestEvent is received, addressed to this URI. This is the
     * object returned by this method. 2) The wait timeout period specified by
     * the parameter to this method expires. Null is returned in this case. 3)
     * An error occurs. Null is returned in this case.
     * 
     * @param timeout
     *            The maximum amount of time to wait, in milliseconds. Use a
     *            value of 0 to wait indefinitely.
     * @return A RequestEvent or null in the case of wait timeout or error. If
     *         null, call getReturnCode() and/or getErrorMessage() and, if
     *         applicable, getException() for further diagnostics.
     */
    public RequestEvent waitRequest(long timeout)
    {
        ...
        synchronized (reqBlock)
        {
            if (reqEvents.size() == 0)
            {
                ...
                {
                    SipStack
                            .trace("SS.waitRequest() - about to block, waiting");
                    reqBlock.waitForEvent(timeout);
                    SipStack
                            .trace("SS.waitRequest() - we've come out of the block");
                }
                catch (Exception ex)
                {
                    setException(ex);
            ...
            SipStack
                    .trace("SS.waitRequest() - either we got the request, or timed out");
            if (reqEvents.size() == 0)
            {
                setReturnCode(TIMEOUT_OCCURRED);
    ...
    /**
     * This method sends a basic, stateful response to a previously received
     * request. Call this method after calling waitRequest(). The response is
     * constructed based on the parameters passed in. The returned
     * SipTransaction object must be used in any subsequent calls to sendReply()
     * for the same received request, if there are any.
     * 
     * @param request
     *            The RequestEvent object that was returned by a previous call
     *            to waitRequest().
     * @param statusCode
     ...
     * @param toTag
     *            If not null, it will be put into the 'To' header of the
     *            response. Required by final responses such as OK.
     * @param contact
     *            If not null, it will be used to create a 'Contact' header to
     *            be added to the response.
     * @param expires
     *            If not -1, an 'Expires' header is added to the response
     *            containing this value, which is the time the message is valid,
     *            in seconds.
     * @return A SipTransaction object that must be passed in to any subsequent
     *         call to sendReply() for the same received request, or null if an
     *         error was encountered while sending the response. The calling
     *         program doesn't need to do anything with the returned
     *         SipTransaction other than pass it in to a subsequent call to
     *         sendReply() for the same received request.
     */
    ...
    {
        return sendReply(request, statusCode, reasonPhrase, toTag, contact,
            ...
            ArrayList additionalHeaders, ArrayList replaceHeaders, String body)
    {
        ...
        try
        {
            response = parent.getMessageFactory().createResponse(statusCode,
                    request.getRequest());
            ...
            if (contact != null)
            {
                response.addHeader(parent.getHeaderFactory()
            ...
            ArrayList additionalHeaders, ArrayList replaceHeaders)
    {
        ...
        {
            return sendReply(request, statusCode, reasonPhrase, toTag, contact,
                    expires, toHeader(additionalHeaders, contentType,
     ...
     * Call this method after calling waitRequest(). The returned SipTransaction
     * object must be used in any subsequent calls to sendReply() for the same
    ...
    public SipTransaction sendReply(RequestEvent request, Response response)
    {
        ...
        if (trans == null)
        {
            ...
            {
                /*
                 * TransactionAlreadyExistsException - this can happen if a
                 * transaction already exists that is already handling this
     ...
     * The returned SipTransaction object must be used in any subsequent calls
     * to sendReply() for the same received request, if there are any.
     * 
     * @param transaction
     *            The SipTransaction object returned from a previous call to
     *            sendReply().
     * @param statusCode
     *            The status code of the response to send (may use SipResponse
     *            constants).
    ...
    public SipTransaction sendReply(SipTransaction transaction, int statusCode,
            String reasonPhrase, String toTag, Address contact, int expires)
    {
        return sendReply(transaction, statusCode, reasonPhrase, toTag, contact,
                expires, null, null, null);
    }
            ...
            ArrayList additionalHeaders, ArrayList replaceHeaders)
    {
        try
        {
            return sendReply(transaction, statusCode, reasonPhrase, toTag,
                    contact, expires, toHeader(additionalHeaders, contentType,
                            contentSubType), toHeader(replaceHeaders), body);
        }
        catch (Exception ex)
        {
    ...
    protected ArrayList toHeader(ArrayList strings) throws Exception
    {
        ...
        while (i.hasNext())
        {
            ...
            Header hdr = parent.getHeaderFactory().createHeader(
    ...
    public SipTransaction sendReply(SipTransaction transaction, int statusCode,
            ...
            ArrayList additionalHeaders, ArrayList replaceHeaders, String body)
    {
    	Response response = createResponse(transaction, statusCode,
                reasonPhrase, toTag, contact, expires,
                additionalHeaders, replaceHeaders, body);
    ...
    /**
     * This method is the same as the basic sendReliableReply(SipTransaction, ...)
     * method except that it allows the caller to specify a message body and/or
     * additional JAIN-SIP API message headers to add to or replace in the
     * outbound message. Use of this method requires knowledge of the JAIN-SIP
     * API.
     ...
     *            body bytes.
     */
    public SipTransaction sendReliableReply(SipTransaction transaction, int statusCode,
            String reasonPhrase, String toTag, Address contact, int expires,
            ArrayList additionalHeaders, ArrayList replaceHeaders, String body)
    {
    	Response response = createResponse(transaction, statusCode,
        ...
        return sendReliableReply(transaction, response);
    }

    private Response createResponse(SipTransaction transaction, int statusCode,
            String reasonPhrase, String toTag, Address contact, int expires,
            ArrayList additionalHeaders, ArrayList replaceHeaders, String body)
    {
        initErrorInfo();
            ...
            Response response)
    {
        ...
        if ((transaction == null)
                || (transaction.getServerTransaction() == null))
        {
        ...
        try
        {
            SipStack.dumpMessage("Response before sending out through stack",
                    response);
    ...
    /**
     * This method sends a stateful reliable response to a previously received request.
     * The returned SipTransaction object must be used in any subsequent calls
     * to sendReply() for the same received request, if there are any.
     * 
     * @param transaction
     ...
     *         sendReply() for the same received request.
     */
    public SipTransaction sendReliableReply(SipTransaction transaction,
            Response response)
    {
        initErrorInfo();
        ...
        if ((transaction == null)
                || (transaction.getServerTransaction() == null))
        {
            setErrorMessage("Cannot send reliable reply, transaction information is null");
            setReturnCode(INVALID_ARGUMENT);
            return null;
        }
        ...
        try
        {
            SipStack.dumpMessage("Response before sending out through stack",
                    response);
            transaction.getServerTransaction().getDialog().sendReliableProvisionalResponse(response);
        }
        catch (Exception ex)
        {
            setException(ex);
    ...
    /**
     * The sendUnidirectionalResponse() method sends out a stateless response
     * message. The response is sent out as is.
     * 
     * @param response
     *            The response to be sent out.
     * @return true if the message was successfully sent, false otherwise.
     */
     ...
     * 
     * @param request
     *            The RequestEvent object that contains the request we are
     *            responding to.
     * @param statusCode
     *            The status code of the response to send (may use SipResponse
     *            constants).
     ...
     */
    public boolean sendUnidirectionalResponse(RequestEvent request,
            int statusCode, String reasonPhrase, String toTag, Address contact,
            int expires)
    {
        initErrorInfo();
     ...
     * net.java.sip.communicator.sip.security.SipSecurityManager, its author is
     * Emil Ivov) and slightly modified to fit here. Thanks for making it
     * publicly available. It is licensed under the Apache Software License,
     * Version 1.1 Copyright (c) 2000.
     * 
     * @param method
     *            method of the request being authenticated.
     * @param uri
     *            digest-uri. This is the request uri of the request (ie,
     *            request.getRequestURI().toString()).
     * @param requestBody
     *            the body of the request message being authenticated. IE:
     *            request.getContent()==null?"":request.getContent().toString()
     * @param authHeader
     *            the challenge that we are responding to. The caller should
     ...
     *            Response.PROXY_AUTHENTICATION_REQUIRED).
     * @param username
     *            the name of the user to send to the challenging server
     * @param password
     *            the user's password
    ...
    public AuthorizationHeader getAuthorization(String method, String uri,
            String requestBody, WWWAuthenticateHeader authHeader,
            String username, String password) throws SecurityException
    {
        String response = null;
        try
        {
            response = MessageDigestAlgorithm.calculateResponse(authHeader
                    .getAlgorithm(), username, authHeader.getRealm(),
                    new String(password), authHeader.getNonce(),
        ...
        catch (NullPointerException exc)
        {
            throw new SecurityException(
                    "The received authenticate header was malformatted: "
                            + exc.getMessage());
        }
        ...
        try
        {
            ...
            {
                authorization = parent.getHeaderFactory()
    ...
    /**
     * This method returns the Via Header currently in effect for this user
     * agent, needed for sending requests such as INVITE. By default, it is set
     * to the IP address and port used by this user agent's sip stack. But if
     * the setPublicAddress() has been called on this object, the returned value
     * will reflect the most recent call to setPublicAddress().
     * 
     * @return an ArrayList containing a single element: the
     ...
     * 
     * @deprecated Use getViaHeaders() instead of this method, the term 'local'
    ...
    /**
     * This method indicates if the given return code is an internal SipUnit
     * return code or not.
     * 
     * @param returnCode
     *            the return code in question
     * @return true if the return code is internal to SipUnit, false if it is a
     *         SIP RFC 3261 return code.
     */
    ...
    protected void addRequestListener(String requestMethod,
    ...
    protected void removeRequestListener(String requestMethod,
            RequestListener listener)
    {
        // multiple listeners per method
    ...
    /**
     * Call this method to get the IP address being used by this user agent (ie,
     * the address it is putting in its contact header, via header, etc. when it
     * sends out messages).
     * 
     * @return A String containing the host IP address being used by this user
     *         agent.
     */
