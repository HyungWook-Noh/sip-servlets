...
public class TestNoProxy extends SipTestCase
{
    ...
    // string
    {
        try
        {
            ...
            //Callee send INFO
            a.listenForRequest();
            assertLastOperationSuccess("a listen Request - " + a.format(), a);

            Thread.sleep(10);
            b.sendRequest(Request.INFO);
            assertLastOperationSuccess("b send INFO - " + b.format(), b);
                        
            a.waitForRequest(Request.INFO, 10000);
            assertLastOperationSuccess("a wait INFO - " + a.format(), a);
            SipRequest received_info = a.getLastReceivedRequest();
            assertNotNull(received_info);
            assertEquals("Unexpected request", Request.INFO, 
            		((Request)received_info.getMessage()).getMethod());
            
            Thread.sleep(10);
            a.sendResponseToLastReceivedRequest(Response.OK, "INFO Answer", 0);
            assertLastOperationSuccess("a send 200/INFO" + a.format(), a);
                        
            Thread.sleep(100);
            Response okForInfo = (Response)b.getLastReceivedResponse().getMessage();
            ...
            //Caller send INFO
            Thread.sleep(10);
            a.sendRequest(Request.INFO);
            assertLastOperationSuccess("a send INFO - " + a.format(), a);
            
            b.waitForRequest(Request.INFO, 10000);
            assertLastOperationSuccess("b wait INFO - " + b.format(), b);
            SipRequest callee_received_info = b.getLastReceivedRequest();
            assertNotNull(callee_received_info);
            assertEquals("Unexpected request", Request.INFO, 
            		((Request)callee_received_info.getMessage()).getMethod());

            Thread.sleep(10);
            b.sendResponseToLastReceivedRequest(Response.OK, "INFO Answer", 0);
            assertLastOperationSuccess("b send 200/INFO" + b.format(), b);

            Thread.sleep(100);
            Response okForCalleeInfo = (Response)a.getLastReceivedResponse().getMessage();
            assertNotNull("Default response not sent", okForCalleeInfo);
            assertEquals("Unexpected default reason", Response.OK, 
            		okForCalleeInfo.getStatusCode());
            assertEquals("Unexpected default reason", Request.INFO, 
            		((CSeqHeader)okForCalleeInfo.getHeader(CSeqHeader.NAME)).getMethod());
            ...
            Thread.sleep(100);
            Response okForBye = (Response)b.getLastReceivedResponse().getMessage();
     ...
     *   |------------>>|
     */
    public void testBothSidesWithPrackUpdateCancel()
    {
        try
        {
            SipPhone ub = sipStack.createSipPhone("sip:becky@nist.gov");
            ...
            ArrayList additionalHeader = new ArrayList(1);
            additionalHeader.add(a.getHeaderFactory().createRequireHeader("100Rel"));
            a.initiateOutgoingCall(null, 
            		"sip:becky@nist.gov",
                    properties.getProperty("javax.sip.IP_ADDRESS") + ':' + myPort + '/' + testProtocol, 
                    null,
                    additionalHeader, 
                    null, 
                    null);
            ...
            b.sendIncomingCallReliableResponse(Response.RINGING, null, -1);
            assertLastOperationSuccess("b send Reliable RINGING - " 
            		+ b.format(), b);
            ...
            assertEquals("Unexpected default reason", "Ringing", a
                    .getLastReceivedResponse().getReasonPhrase());
            assertNotNull("RSeq is null", a.getLastReceivedResponse()
            		.getMessage().getHeader(RSeqHeader.NAME));

            Thread.sleep(10);
            a.sendPrack();
            assertLastOperationSuccess("a send PRACK - " + a.format(), a);

            b.waitForPrack(10000);
            assertLastOperationSuccess("b wait PRACK - " + b.format(),
                    b);
            SipRequest received_prack = b.getLastReceivedRequest();
            assertNotNull(received_prack);
            assertEquals("Unexpected request", Request.PRACK, 
            		((Request)received_prack.getMessage()).getMethod());

            Thread.sleep(10);

            b.sendResponseToLastReceivedRequest(Response.OK, "PRACK Answer", 0);
            assertLastOperationSuccess("b send 200/PRACK" + b.format(), b);

            Thread.sleep(100);
            Response okForPrack = (Response)a.getLastReceivedResponse().getMessage();
            ...
            //Callee send UPDATE
            a.listenForUpdate();
            assertLastOperationSuccess("a listen UPDATE - " + a.format(), a);

            Thread.sleep(10);
            b.sendUpdate();
            assertLastOperationSuccess("b send UPDATE - " + b.format(), b);
            
            a.waitForUpdate(10000);
            assertLastOperationSuccess("a wait UPDATE - " + a.format(), a);
            SipRequest received_update = a.getLastReceivedRequest();
            assertNotNull(received_update);
            assertEquals("Unexpected request", Request.UPDATE, 
            		((Request)received_update.getMessage()).getMethod());

            Thread.sleep(10);
            a.sendResponseToLastReceivedRequest(Response.OK, "UPDATE Answer", 0);
            assertLastOperationSuccess("a send 200/UPDATE" + a.format(), a);
            
            Thread.sleep(100);
            Response okForUpdate = (Response)b.getLastReceivedResponse().getMessage();
            assertNotNull("Default response not sent", okForUpdate);
            assertEquals("Unexpected default reason", Response.OK, 
            		okForUpdate.getStatusCode());
            assertEquals("Unexpected default reason", Request.UPDATE, 
            		((CSeqHeader)okForUpdate.getHeader(CSeqHeader.NAME)).getMethod());

/*            //Caller send UPDATE
            Thread.sleep(10);
            a.sendUpdate();
            assertLastOperationSuccess("a send UPDATE - " + a.format(), a);
            
            b.waitForUpdate(10000);
            assertLastOperationSuccess("b wait UPDATE - " + b.format(), b);
            SipRequest received_update = b.getLastReceivedRequest();
            assertNotNull(received_update);
            assertEquals("Unexpected request", Request.UPDATE, 
            		((Request)received_update.getMessage()).getMethod());

            Thread.sleep(10);
            b.sendResponseToLastReceivedRequest(Response.OK, "UPDATE Answer", 0);
            assertLastOperationSuccess("b send 200/UPDATE" + b.format(), b);

            Thread.sleep(100);
            Response okForUpdate = (Response)a.getLastReceivedResponse().getMessage();
            assertNotNull("Default response not sent", okForUpdate);
            assertEquals("Unexpected default reason", Response.OK, 
            		okForUpdate.getStatusCode());
            assertEquals("Unexpected default reason", Request.UPDATE, 
...
*/            
            Thread.sleep(100);
            a.sendCancel();
            assertLastOperationSuccess("a send CANCEL - " + a.format(), a);

            b.waitForCancel(10000);
            assertLastOperationSuccess("b wait CANCEL - " + b.format(), b);
            ...
            Thread.sleep(100);
            Response okForCancel = (Response)a.getLastReceivedResponse().getMessage();
            assertNotNull("Default response not sent", okForCancel);
            assertEquals("Unexpected default reason", Response.OK, 
            		okForCancel.getStatusCode());
            assertEquals("Unexpected default reason", Request.CANCEL, 
            		((CSeqHeader)okForCancel.getHeader(CSeqHeader.NAME)).getMethod());

            Thread.sleep(100);
            b.sendIncomingCallResponse(Response.REQUEST_TERMINATED, 
            		"Answer 487- Hello world", 0);
            assertLastOperationSuccess("b send 487 - " + b.format(), b);

            a.waitOutgoingCallResponse(100);
            assertLastOperationSuccess("a wait 2nd response - " + a.format(), a);
            assertEquals("Unexpected 2nd response received", 
            		Response.REQUEST_TERMINATED, a.getReturnCode());
