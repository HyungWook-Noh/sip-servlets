...
/**
 * This class tests two SipStacks on the same machine. You need a different IP
 * address for each one (set the appropriate property below, or modify the
 * run-tests.xml script to set it).
 * 
...
public class TestTwoSipStacksNoProxy extends SipTestCase
{
    ...
    static
    {
        ...
        defaultProperties2.setProperty("javax.sip.IP_ADDRESS", "192.168.1.102");
    ...
    public TestTwoSipStacksNoProxy(String arg0)
    {
        ...
        String prop = input_props.getProperty("sipunit.ipaddress.1");
        if (prop != null)
        {
            properties1.setProperty("javax.sip.IP_ADDRESS", prop);
        }

        prop = input_props.getProperty("sipunit.ipaddress.2");
        if (prop != null)
        {
            properties2.setProperty("javax.sip.IP_ADDRESS", prop);
        }

        prop = input_props.getProperty("sipunit.testport.1");
        if (prop != null)
        {
            try
            {
    ...
    public void testBothSides()
    {
        try
        {
            ...
            a.initiateOutgoingCall("sip:becky@nist.gov", properties2
